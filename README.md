...
Het uitgangspunt: één cijfer op het startscherm — hoeveel kcal je nog over hebt —
en al het andere een niveau dieper. Loggen kost één tik: het logvel opent op je
...
- Account met synchronisatie over je toestellen (optioneel — zonder account
  blijft alles lokaal).
...
> de simulator was tijdens het schrijven niet beschikbaar.
...
**Offline-first.** Isar is de waarheid op het toestel. De synchronisatie
...
**Privacy.** Zonder account verlaat er niets je toestel. Met een account staan
je gegevens achter row level security op `auth.uid()` — niemand kan bij een
ander z'n logboek. "Account verwijderen" wist de sessies, de refresh-tokens en
de gebruiker, en alles wat eraan hangt verdwijnt via `on delete cascade`.
...
**Codegeneratie.** Isar-collecties, Riverpod-providers en freezed-modellen
worden gegenereerd. Draai `dart run build_runner build` na elke wijziging in
...
| | |
| --- | --- |
| `001`–`006` | Productcatalogus: NEVO, merkproducten, zoekindex en ranking |
| `007` | Gebruikersgegevens: logboek, doelen, gewicht, water, eigen producten, recepten, herinneringen — alles achter RLS |
| `008`, `011` | `delete_account()`: verwijdert sessies, tokens en de gebruiker |
...
[Open Database License](https://opendatacommons.org/licenses/odbl/). De volledige
vermelding staat in `assets/legal/nevo_attribution.txt`.
